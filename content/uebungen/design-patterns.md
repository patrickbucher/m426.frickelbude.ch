...
Erstelle für diese Übung einen Fork vom Repository [m426-2026/design-patterns](https://github.com/m426-2026/design-patterns) und klone diesen.
...
Studiere den Code in den Dateien `geometry.ts`, `geometry_third_party.ts` und `geometry_test.ts`.
...
In `parking_lot.ts` und `parking_lot_simulation.ts` ist eine Parkhaussimulation implementiert, die folgendermassen gestartet werden kann:

    deno parking_lot_simulation.ts
...
Baue die Simulation foldendermassen um:
...
1. Das Parkhaus soll als _Publisher_ agieren und seine _Subscriber_ jeweils informieren, wenn ein Auto in das Parkhaus einfährt oder es verlässt.
   - Erstelle hierzu zwei Interfaces `Publisher` und `Subscriber` mit den entsprechenden Methoden.
...
3. Überlege dir, ob die Ereignisse als eine Reihe von Variablen (Name des Parkhauses, Füllstand usw.) übermittelt werden sollen, oder ob hier ein spezielles `Event`-Objekt hilfreich wäre.
...
Entwurfsmuster: [State](https://refactoring.guru/design-patterns/state) (_behavioral_)

In `texteditor.ts` und in `texteditor.html` ist ein Text-Editor implementiert, der Texteingaben als Pseudo-Dateien im `localStorage` des Browsers ablegt.
...
**Wichtig**: Der TypeScript-Code muss nach jeder Änderung nach JavaScript transpiliert werden. Hierzu ist ein Deno-Task vorkonfiguriert, der folgendermassen gestartet werden kann:

    deno task texteditor

Nimm deine Änderungen nur im TypeScript-Code (`texteditor.ts`) vor und nicht im generierten JavaScript-Code (`texteditor.js`).